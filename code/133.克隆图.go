...
// 			return seen[node]
// 		}
// 		clone := &Node{node.Val, make([]*Node, len(node.Neighbors))}
// 		seen[node] = clone
// 		for i, ne := range node.Neighbors {
// 			clone.Neighbors[i] = traverse(ne)
// 		}
// 		return clone
// 	}
...
func cloneGraph(node *Node) *Node {
	...
	seen := map[*Node]*Node{}
	queue := []*Node{node}
	seen[node] = &Node{node.Val, make([]*Node, 0, len(node.Neighbors))}
	...
	for len(queue) > 0 {
		...
		for _, ne := range curr.Neighbors {
			if _, ok := seen[ne]; !ok {
				seen[ne] = &Node{ne.Val, make([]*Node, 0, len(ne.Neighbors))}
				queue = append(queue, ne)
			}
			seen[curr].Neighbors = append(seen[curr].Neighbors, seen[ne])
		}
	}
	return seen[node]
}
