...
func lengthLongestPath(input string) int {
	...
	max := 0
	length := 0
	lv := 1 // start from lv 1 to prevent check error for prevCount := lvlCount[lv-1]
	for _, c := range input {
		switch c {
		case '\n':
			prevCount := lvlCount[lv-1]
			if isFile {
				newLen := prevCount + length
				if max < newLen {
					max = newLen
				}
				isFile = false
				...
				newLen := prevCount + length + 1 // addtional 1 is for /
				if lv >= len(lvlCount) {
					lvlCount = append(lvlCount, newLen)
				} else {
					lvlCount[lv] = newLen
				}
			}
